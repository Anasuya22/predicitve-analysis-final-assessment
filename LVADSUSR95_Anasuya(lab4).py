...
Original file is located at
    https://colab.research.google.com/drive/13OLz0oL6oJeWMcEngPXM1mvhVwlOsc0C
"""
...
Based on time,location,amount anomaly detection is done.
If the amount value is very less or very high then it is considered as anomaly.

calucated count,mean,std,max,min are found.
From there we can be able to see count is same for all the 4 which is transactionid, time,user is 1000.
min value for all the four is around 1.
Anamoly for transactionID,amount,time and user is plotted as a graph so that we can be able to infer that how anamoly detection is done using isolation forest"""
